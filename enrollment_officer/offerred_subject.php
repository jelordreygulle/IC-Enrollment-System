...
<div class="container">
	<?php
		...
		<div class="well">
				...
				@$course 	 =  $_POST['course'];
				  	  ...
				  	  If (isset($_POST['search'])){
				  	  	...
				  	  	if ($term == ''  AND $course==''  AND $ay == ''){
				  	  		...
				  	  		$mydb->setQuery("SELECT `tbl_offerred_subject` . *  , `tbl_course` . * , `tbl_subject` . *, `tbl_offerred_subject`.`AY` as ay_ko, `tbl_offerred_subject`.`semester` as term_ko, `tbl_offerred_subject`.`course_id` as course_niya
				  		...
				  		function loadresult(){
							...
							foreach ($cur as $result) {
						  		...
						  		if($_SESSION['course'] == $result->course_niya){
						  			echo '<td><input type="checkbox" name="selector[]" id="selector[]" value="'.$result->offerred_subject_id. '"/>
						  				<a href="edit_offered_subject.php?id='.$result->offerred_subject_id.'">' . $result->subject_code.'</a></td>';
				  ...
				  <tfoot>
					...
					if ($pagination->total_pages() > 1){
						...
						if ($current_page == 1 ){
							echo ' <li class="disabled"><a href=offerred_subject.php?page='.$pagination->First_page().'>First </a> </li>';
						
						}else{
							echo ' <li ><a href=offerred_subject.php?page='.$pagination->First_page().'>First </a> </li>';

						}
						
						if  ($current_page >= 1 ){
							
							echo ' <li> <a href=offerred_subject.php?page='.($current_page - 1).'>Previous </a> </li>';

						}else{
							echo ' <li class="disabled"> <a href=offerred_subject.php?page='.($current_page - 1).'>Previous </a> </li>';
						}
						
						if ($current_page <  $pagination->total_pages()){
						
							echo ' <li><a href=offerred_subject.php?page='.($current_page + 1) .'>Next</a></li> ';
											
						}else{

							echo ' <li class="disabled"><a href=offerred_subject.php?page='.($current_page + 1) .'>Next</a></li> ';
						}
						...
						if ($current_page ==  $pagination->total_pages() ){
							...
							echo ' <li class="disabled"><a href=offerred_subject.php?page='.$pagination->total_pages().'>Last </a> </li>';
						}else{
							echo ' <li><a href=offerred_subject.php?page='.$pagination->total_pages().'>Last </a> </li>';
						}
				...
				<?php
				$querya ="SELECT `ay` , `term`, `ay_status`, `ay_id`
						FROM  `tbl_ay` 
						WHERE  `ay_status` =  'ACTIVE'
						LIMIT 0 , 30";
				$resa = mysql_query($querya) or die ("Could not execute query2.");
				...
				if($row2a == 1){
					...
					echo '<a href="newofferedsubject.php?ay='.$ayy2.'&term='.$tt.'" class="btn btn-success"><span class="glyphicon glyphicon-plus-sign"></span> New Subject Offered </a>';
					
				}
				else if($row2a > 1){

					echo '<a href="newofferedsubject.php" disabled class="btn btn-success"><span class="glyphicon glyphicon-plus-sign"></span> New Subject Offered </a>***Offering a new subject is disable because the current AY and Term is conflict with the other active AY & Term. Please contact your administrator';
					
				}

				else{

					echo '<a href="newofferedsubject.php" disabled class="btn btn-success"><span class="glyphicon glyphicon-plus-sign"></span> New Subject Offered </a>***Offering a new subject is disable because the current AY and Term is already disabled';
