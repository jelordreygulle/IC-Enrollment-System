...
class officer{
	...
	protected static $tbl_name = "tbl_officer_incharge";
	function db_fields(){
		global $mydb;
	...
	static function Authenticateofficer($email="", $h_upass="", $accademic ="", $sem=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tbl_officer_incharge` WHERE `officer_username`='$email' and `officer_password`='$h_upass'
						LIMIT 1
						");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $found_user = $mydb->loadSingleResult();
		    $_SESSION['officer_incharge_id'] = $found_user->off_incharge_id;
            $_SESSION['fullname']    = $found_user->fullname;
            $_SESSION['postion']= $found_user->position;
            $_SESSION['SY']= $found_user->officer_status;
            $_SESSION['officer_username']    = $found_user->officer_username;
            $_SESSION['officer_password']    = $found_user->officer_password;
            $_SESSION['accademic']   = $accademic;
			$_SESSION['term']   = $sem;
