...
class assessor{
	...
	protected static $tbl_name = "tbl_assessor";
	function db_fields(){
		global $mydb;
	...
	static function Authenticateassessor($email="", $h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tbl_assessor` WHERE `assessor_username`='$email' and `assessor_password`='$h_upass'
						LIMIT 1
						");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $found_user = $mydb->loadSingleResult();
		    $_SESSION['assesor_id'] = $found_user->assessor_id;
            $_SESSION['fullname']    = $found_user->fullname;
            $_SESSION['gender']= $found_user->gender;
            $_SESSION['address']= $found_user->address;
            $_SESSION['assessor_username']    = $found_user->assessor_username;
            $_SESSION['assessor_password']    = $found_user->assessor_password;
