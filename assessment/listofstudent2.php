...
<div class="container">
	...
	?>
		<div class="well">
				  ...
				  <tbody>
					...
					$pagination = new StudPagination($current_page, $per_page, $total_count);
				  	  ...
				  	  if ($idno == '' AND $lname=='' AND $fname == ''){
				  	  	$mydb->setQuery("SELECT DISTINCT  `tbl_student`.`student_id` , CONCAT(  `tbl_student`.`lname` ,  ', ',  `tbl_student`.`fname` ,  ' ', `tbl_student`.`mname` ) AS  'Name',  `tbl_course`.`course_code` ,  `tbl_student`.`course_id` ,  `tbl_course`.`course_id` , `tbl_course`.`Major`
				  		...
				  		function loadresult(){
							...
							foreach ($cur as $student) {
					  		...
					  		echo '<td><a href="./edit_studentinfo.php?id='.$student->student_id.'">' . $student->student_id.'</a></td>';
					  		echo '<td>'. $student->Name.'</td>';
					  		echo '<td>'. $student->course_code.'-'.$student->Major.'</td>';
					  		
					  		echo '<td><a href = "listofassessment2.php?studentId='.$student->student_id.'" >Assessment Records</a></td>';
					  		echo '</tr>';
					  		}
				...
				<div class="btn-group">
				  <a href="filter_assess_student.php" class="btn btn-danger"><span class="glyphicon glyphicon-search"></span>  Search</a>
